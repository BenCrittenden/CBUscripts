...
%Conditions: no switch, within group, between group, but 2 to start with,
%to check for handedness
%therefore, five conditions in total.

condition_codes = [71 72 73 74 75 76 81 82 83 84 85 86 91 92 93 94 95 96];
 for StimType = condition_codes
...
for sub = 1:nsubs
    ...
    for sess = 1:nsess
        ...
        %movement_parameters = load(move_param_file); % load the movement parameters
            ...
            c_swtt = timings(:,7); %swtich&tasks combined into 18 types.
            
                            
            %Event related design
            c_eons = c_eons(isfinite(c_edur));
            c_st = c_st(isfinite(c_edur));
            c_tt = c_tt(isfinite(c_edur));
            c_swtt = c_swtt(isfinite(c_edur));
            c_edur = c_edur(isfinite(c_edur));
                                      
            %SUBTRACT 8 DUMMY TRS 
            c_eons = c_eons - 8; % may need to change this

            % now set into SPM design
            for cno = condition_codes;
                               
                descol = descol+1;
                nm = {condnames{cno}}; %nm means name, it loads the name of condition 1,2,3...
                tmp = (c_swtt == cno); % if c_typ = cno, tmp = 1. If not, tmp = 0
